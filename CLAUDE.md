...
### Changelog
- `v3.92` — **선불권 오른쪽 헤더 글자를 다른 탭과 동일하게.** 눈썹이 전용 `.pp-eyebrow`(굵기 700·자간 .5px)라 'RECEIPT DB'가 더 굵고 좁았고, 제목에 전용 `line-height`(1.15/1.18)가 걸려 제목·부제 위치가 1~2px 달랐다 → 눈썹을 공통 `.main-eye`로 바꾸고 선불권 전용 눈썹·제목 행간 규칙 삭제. 측정 결과 데스크탑(1280) 눈썹·버전·제목·부제의 위치·높이·크기·굵기·자간·색이 설정·추가 탭과 동일, 모바일(390) 상세 헤더는 영수증 상세와 동일. ⚠️ 선불권 헤더에 전용 글자 규칙을 다시 만들지 말 것 — 공통 `.main-eye`/`.main-title`/`.main-sub`만 쓴다. 변경 파일 `index.html`·`prepaid.js`·`prepaid.css`·`CLAUDE.md`.
