...
- 경로는 `_dbxScanDir()`이 결정한다. 기본값 `/스캔함`(앱 최상단), 설정에서 변경 가능(localStorage `dbx_scan_dir`).
  ⚠️ **데이터 폴더(`_DBX_DATA_DIR`) 안에 두지 말 것** — App folder 스코프에서 경로가 두 번 중첩돼
  Dropbox 앱으로 접근하기가 매우 번거로워진다 (v2.32에서 밖으로 뺀 이유).
