...
- `v3.90` — **단둘이 결제 목록 + 뒤로가기 '엉뚱한 곳' 근본 수정 + 숫자 글꼴 Dutch Pay와 동일. ⚠️ 계산·schema·저장·Dropbox·Dutch Pay 불변.**
  ...
  **검증**(실데이터 151, 1280·390·430): 단둘이 목록 19건·합계·필터 · 목록→영수증→Back 1 = 같은 목록·필터 / Back 2 = 사람 화면 · 유령 시트 뒤 Back 1번에 사람 화면(v3.89 FAIL → PASS) · 빠른 연속 Back · v3.84 뒤로가기(기대값: drill 복귀로 갱신)·v3.83/87 차수·v3.86 결제자·v3.89 요약·v3.72/73 오버레이 회귀 PASS, overflow 0·콘솔 에러 0. ⚠️ iOS Safari 실기기 글꼴은 이 환경(Linux)에서 확인 불가 — 사용자 확인 필요. 변경 파일 `index.html`·`CLAUDE.md`.
