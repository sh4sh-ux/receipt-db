...
- ⚠️ **`sw.js`의 `CACHE_NAME`도 매 릴리스 `APP_VERSION`과 같이 올릴 것**(`scripts/check_app.py`가 확인). v3.06~v3.96 동안 안 올려 아이폰이 옛 선불권 파일을 계속 썼다(v3.97). 셸 파일은 network-first라 새 파일 반영은 캐시 이름과 무관하지만, sw.js 내용이 바뀌어야 폰이 새 서비스 워커를 설치한다.
...
### Changelog
- `v3.98` — **모바일 헤더 아래 구분선을 모든 탭 같은 높이로(112px, safe-area 0 기준).** 사용자 아이폰 캡처를 재 보니 눈썹·제목 위치는 모든 탭이 같았고(아이폰 전용 차이는 v3.97의 옛 선불권 파일뿐), **구분선만** 추가 108 · 통계/설정/상세 124 · 내역/사람/선불권 157로 달랐다(같은 값이 크롬에서도 재현 — 기기 차이가 아니라 레이아웃). 원인: `--mobile-head-h`가 옛 '부제 두 줄' 기준 124, 추가 탭만 따로 108, 목록형은 제목 아래 `.app-title` margin 16px + `.side-top` 아래 여백 10px + 월 이동 줄 56px. 수정(모바일만, `<style>` 끝 v3.98 블록): `--mobile-head-h` 98+safe(=112), 추가 탭 전용 높이 삭제, 목록형은 제목 아래 여백 0 + 월 이동·'전체'·총 잔액 줄 46px → 모든 탭 구분선 112(목록형 111.9). 눈썹 14·제목 39·부제 72 그대로, 데스크탑 불변(H144·30/50/84). 검증 390·375·430 7개 화면 측정 · 회귀(add95t·v84·v90·pp91t) PASS · 375 입력 후 저장 버튼 보임 · check_app OK. 변경 파일 `index.html`·`sw.js`·`README.md`·`CLAUDE.md`.
- `v3.97` — **아이폰에서 탭마다 헤더·선불권 화면이 다르던 원인 = 서비스 워커 캐시.** `sw.js`가 `prepaid.js`·`prepaid.css`·아이콘을 **cache-first**로 주고 캐시 이름이 `v3.06`에 멈춰 있어(`check_app.py`가 계속 FAIL을 냈음), 한 번 설치된 폰(아이폰 홈 화면 앱)은 index.html만 최신이고 선불권 JS·CSS는 설치 당시 것을 계속 썼다 → 선불권이 v3.91 이전 화면, 헤더 규칙(v3.92~93)이 선불권에 안 걸림. 안드로이드는 나중에 설치했거나 캐시가 비어 최신이었음. **수정**: 모든 셸 파일 **network-first**(파일은 `cache:'no-cache'`로 HTTP 캐시도 재확인, 끊겼을 때만 캐시) + `CACHE_NAME`을 앱 버전과 같이(`v3.97`), README 버전도 맞춰 `check_app.py` 통과. **검증**(옛 SW 설치 상태에서 업그레이드 시뮬레이션): 옛 SW는 바뀐 prepaid.js를 안 줌(재현) → 새 sw.js 배포 후 첫 로드에 새 SW 설치·옛 캐시 삭제 → 다음 로드부터 최신 → 이후 변경도 즉시 → 오프라인에서도 앱 실행. ⚠️ 폰은 **배포 후 앱을 두 번 열어야**(첫 번째에 새 SW 설치, 두 번째부터 새 파일) 반영된다. 변경 파일 `sw.js`·`index.html`·`README.md`·`CLAUDE.md`.
- `v3.96` — **추가 화면 [AI 붙여넣기|직접 입력] 버튼이 답답해 보이던 것 수정.** v3.94에서 버튼 줄을 `flex:0 0 auto`로 바꾸면서 버튼 좌우 여백이 0(`padding:7px 0`)이라 글자가 가장자리에 붙어 있었다(71·77px) → 두 버튼 같은 폭 + 좌우 12px·높이 34px(`.addx-top .input-mode-row{flex:0 1 280px}`, PC 135px씩 · 390 105px · 375 98px), 프롬프트 복사 버튼은 줄바꿈 없이 고정. 붙여넣기 전 빈 미리보기 자리(빈 칸 24px)도 숨김(`:has(.empty-state:only-child)`). 검증 1280·390·375 · add95t ALL PASS · 375 저장 버튼 보임. 변경 파일 `index.html`·`CLAUDE.md`.
- `v3.95` — **추가 화면 = 영수증 상세와 같은 화면(칸·순서·모양 동일).** v3.94는 배치만 옮겨 '영수증 확인' 카드(매장·날짜·총액·품목 줄)와 참석자 [+ 나][+ 사람] 버튼이 남아 상세와 모양이 달랐다 → 상세 마크업·클래스를 그대로 씀.
  순서 = [AI 붙여넣기|직접 입력] + 프롬프트 복사 → 붙여넣기 칸(붙이면 한 줄로 접힘, [원문]) → **상단 카드**(상세의 `.det-hd-card`: `매장 · ○○ 결제 … 총액 ▾ / 참석자`, 누르면 품목 편집기 + **합계**(총액 칸) 펼침, 총액이 없으면 '금액 입력'(파랑)으로 펼친 채) → **'정보' 상자**(`.detail-info-card`: 매장명 `#addStore` / 일자 `#addDate` · 카테고리 / 결제자 드롭다운 `#addPayerSel`(+hidden `#payerInp`) · 결제수단 / **참석자 = 쉼표 입력칸 `#participantsInp`** / 분담 방식 🎉 한턱 + 칩(`_makeSplitUI`, 상세와 같은 옵션) + 사진) → 메모·태그.
  **제거**: '영수증 확인' 카드(qc 줄)·[+ 나][+ 사람]·'참석자 · 분담' compact 블록(`_peopleBlockHtml`/`_bindPeopleUI`는 이제 호출처 없음). **직접 입력**도 같은 정보 상자·사진칸을 쓰고 상단 카드의 금액 자리만 입력칸(`#directTotal`), 자체 상호명·날짜·사진칸 제거(`_directPhotoBlob` → `pendingPhotoBlob`).
  매장명·일자 칸은 `_currentParsed`와 연결(입력 → `p.store`/`p.date`, 파싱 시 칸 채움, 원문에 없으면 칸에 적어 둔 값 유지, 저장 직전 한 번 더 확정). 필수칸이 비면 칸 테두리 파랑(`.addx-miss`). 품목 합계가 총액과 맞으면 '품목 합계' 줄 숨김. ⚠️ 파서·`validateParsed`·저장 경로(`saveBtn`·`_saveDirectEntry`)·schema·계산 불변.
  ⚠️ CSS 주석 안에 `.det-hd-*/`처럼 `*/`가 들어가면 주석이 거기서 끝나 **바로 다음 규칙이 통째로 무시된다**(이번에 `.addx{display:flex}`가 죽어 간격 0으로 보였음) — 주석에 `*` 뒤 `/`를 쓰지 말 것.
  **검증**(실데이터 151, 1280·390 + 375 다크): 정보 칸 순서 = 상세 · [+ 나][+ 사람] 없음 · 결제자 기본 나 · 붙여넣기 → 칸 채움·현금·저장 활성 · 카드 탭 → 품목 펼침 · 매장명·참석자·결제자 수정 → 카드 반영 · 한턱·메모 → 저장 → DB 일치 → 상세 재오픈 같은 값 · 저장 후 칸 비움 · 총액 없음 → '금액 입력'·입력 → 카드 반영 · 매장명 비움 → 저장 비활성 · 직접 입력(일자 오늘·사진칸·참석자) 저장 · 375 끝까지 스크롤·입력 후 저장 버튼 보임 · overflow 0 · 콘솔 에러 0, 회귀(v3.83·84·86·87·90·91) PASS. 변경 파일 `index.html`·`CLAUDE.md`.
- `v3.94` — **추가 화면을 영수증 상세(내역)와 같은 배치·동선으로. ⚠️ 입력칸 id·`parseReceiptText`·`validateParsed`·저장 경로(`saveBtn`·`_saveDirectEntry`)·참석자 블록(`_peopleBlockHtml('add')`)·schema 전부 불변 — 기존 요소를 옮겨 배치만 바꿈.**
  순서 = [AI 붙여넣기|직접 입력] + 프롬프트 복사 → **붙여넣기 칸**(붙여넣어 파싱되면 `#gptModeArea.qc-parsed`로 '✓ 붙여넣은 내용으로 채웠어요 · 품목 N개 · 합계 일치 [원문]' 한 줄로 접힘, [원문]=`.addx-raw`로 다시 열어 수정) → **영수증 확인 카드**(매장·날짜·총액·품목, v3.79 그대로) → **'정보' 상자**(상세의 `.detail-info-card`와 같은 회색 상자: 왼쪽 결제수단·카테고리 2칸 + 경조사 칸 + 참석자·분담·결제자·한턱 / 오른쪽 영수증 사진) → **메모·태그**(항상 보임). v3.80의 '추가 정보' 접힘 줄은 없앰(JS 호환용 빈 `#qcMore`만 남김). 붙여넣기 전 빈 미리보기 안내는 숨김(파싱 오류 안내는 보임). 직접 입력은 자체 사진칸을 쓰므로 정보 상자 사진칸 숨김(`.addx-direct`). 제목 '새 영수증'(직접 입력은 '새 영수증 · 직접 입력').
  **저장 위치**: 데스크탑은 하단 저장 바를 숨기고 헤더 `[초기화][저장]`(`#addHdrClear`/`#addHdrSave` — 영수증 상세 [되돌리기][저장]과 같은 자리·제목 줄 가운데, 누르면 기존 `#clearBtn`/`#saveBtn` 클릭, disabled는 `_setAddSaveState`+MutationObserver로 동기), 저장 안내는 헤더 부제에. 모바일은 기존 하단 저장 바 그대로(키보드 안전 — v2.64 규칙).
  **검증**(1280·390): 붙여넣기 전 저장 비활성 · 붙여넣기 → 한 줄 접힘·결제수단 현금·저장 활성 · [원문] 토글 · 저장 → DB(총액·결제수단·품목·날짜) · 초기화 · 직접 입력 저장 · 참석자·결제자 저장(addsmoke) · 헤더 버튼 제목 줄 가운데(64.4) · overflow 0 · 콘솔 에러 0, 뒤로가기(v3.84)·결제자(v3.86)·단둘이(v3.90) 회귀 PASS. ※ 옛 추가 화면 테스트(v3.79~v3.82)는 v3.81~84에서 바뀐 UI(추천 칩·상세 참석자 칸)를 찾아 원래부터 실패. 변경 파일 `index.html`·`CLAUDE.md`.
- `v3.93` — **헤더 규칙화: 좌측 패널 헤더 = 오른쪽 헤더, 영수증 상세 포함 모든 탭 동일(위 「헤더 규칙」 섹션 신설).** `<style>` 끝에 Header Contract 블록 하나로 좌(`.side-top`)·우(`.main-top`) 눈썹·버전·제목 글자와 여백을 같은 값으로. 좌측 제목 23→24px(PC)·눈썹/제목 3px·2px 내려가 있던 것 정렬(화면 절대 y 동일: PC 눈썹 110·버전 108·제목 130 / 모바일 84·84·109). 영수증 상세 PC 제목이 ‹ 돌아가기 버튼 때문에 6px 아래(56)였던 것 → 눈썹 자리 버튼 줄 높이 13px(누르는 영역은 유지) + 숨긴 버전 자리 유지로 50. 모바일 상세형(영수증·사람·선불권) 제목 46→39로 목록형과 같게. **측정**: PC 통계·영수증 상세·사람·추가·선불권·설정 제목 50·부제 84, 모바일 39·72 전부 동일. **헤더 버튼도 제목 줄 가운데**: 영수증 상세 버튼이 제목보다 18px 위(PC 46 / 모바일 34)에 있던 것 → PC 64.4 · 모바일 52.6(통계 기간·사람 '전체 기간'과 같은 줄). 회귀(뒤로가기 v3.84·차수 v3.83·결제자 v3.86·사람 요약 v3.89·단둘이 v3.90·선불권 v3.91) PASS, 콘솔 에러 0. 변경 파일 `index.html`·`CLAUDE.md`.
- `v3.92` — **선불권 오른쪽 헤더 글자를 다른 탭과 동일하게.** 눈썹이 전용 `.pp-eyebrow`(굵기 700·자간 .5px)라 'RECEIPT DB'가 더 굵고 좁았고, 제목에 전용 `line-height`(1.15/1.18)가 걸려 제목·부제 위치가 1~2px 달랐다 → 눈썹을 공통 `.main-eye`로 바꾸고 선불권 전용 눈썹·제목 행간 규칙 삭제. 측정 결과 데스크탑(1280) 눈썹·버전·제목·부제의 위치·높이·크기·굵기·자간·색이 설정·추가 탭과 동일, 모바일(390) 상세 헤더는 영수증 상세와 동일. ⚠️ 선불권 헤더에 전용 글자 규칙을 다시 만들지 말 것 — 공통 `.main-eye`/`.main-title`/`.main-sub`만 쓴다. **같이 통일**: ① 통계(가계부 요약) 제목이 옆 기간 버튼(38px) 때문에 줄이 커져 3px 아래에 있던 것 → `.main-title-row .ledger-period-slot{margin-block:-6px}`(버튼은 제목 줄 가운데, 줄 높이는 제목 기준) ② 사람 상세 데스크탑 눈썹이 사람 아이콘이라 1~2px 어긋나던 것 → 'RECEIPT DB' 글자로 ③ 모바일 사람 상세(‹ 사람)는 영수증·선불권 상세처럼 버전 숨김. **측정**: 데스크탑 1280 설정·추가·통계·사람·선불권 모두 눈썹 y30·버전 y28·제목 y50·부제 y84(헤더 144), 모바일 390 목록형(설정·통계) 제목 y39 / 상세형(영수증·사람·선불권) 제목 y46으로 각각 동일. 변경 파일 `index.html`·`prepaid.js`·`prepaid.css`·`CLAUDE.md`.
- `v3.91` — **선불권 화면을 다른 레일과 같은 3단(레일 | 좌측 목록 | 오른쪽 상세)으로. ⚠️ 계산(`ppTotals`·`ppLatestUse`·`ppSuggestedUseAmount`)·저장(`ppCommit`·`ppWalletForm`·`ppEventForm`)·Dropbox 병합 불변 — 배치만.**
  ① **좌측 = 사람 모드(v3.78)와 같은 grammar**: `.side.pp-mode`(`_applyPrepaidModeChrome`) — 제목 '선불권' · month-nav 자리에 범위 행 `#ppScope`(총 잔액 + `+ 선불권 등록` — 좌/우 헤더 구분선 정렬 유지) · 같은 `#searchInp`(선불권 모드에선 `_ppListQ`로 매장명·초성만 거름, 영수증 검색어와 분리) · 툴바 `선불권 N개` + `#ppSortSel2`[잔액 높은순|최근 사용순|이름순](`ppListSort`) · `.r-card.ppw-row` = **내역 목록과 같은 행**(매장명 … 잔액 / `.r-card-meta` 카테고리 · 최근 YYYY.MM.DD or 유효기간 지남(빨강), 막대 없음·행 높이·선택 표시(왼쪽 파란 선, 모서리 직선) 동일). ⚠️ `.pp-row`는 사람별 분담 행(`border-radius` 있음)이 이미 쓰는 클래스라 겹치면 선택 행이 둥글어진다 — 선불권 행은 `ppw-row`. `renderSide`가 선불권 탭이면 `renderPrepaidSide`로, 다른 탭으로 가면 좌측 원복(`_ppChromeWasOn`).
...
- `v3.90` — **단둘이 결제 목록 + 뒤로가기 '엉뚱한 곳' 근본 수정 + 숫자 글꼴 Dutch Pay와 동일. ⚠️ 계산·schema·저장·Dropbox·Dutch Pay 불변.**
  ...
  ③ **목록에서 연 영수증 → Back = 그 목록(같은 필터)으로 복귀**: 사람 요약 drill(전체 결제·여럿이·단둘이 결제·한턱·참석 부담액 등, `route.mtg={kind:'pd',type,filter}`)과 단둘이 한턱 drill(`kind:'duo'`)도 만남 요약(v3.84)과 같은 방식. breadcrumb = 목록 제목.
  ④ **숫자 쉼표**: 전역 글꼴 스택을 Dutch Pay 금액 글꼴(`system-ui` 우선)로(위 「숫자 규칙」).
  **검증**(실데이터 151, 1280·390·430): 단둘이 목록 19건·합계·필터 · 목록→영수증→Back 1 = 같은 목록·필터 / Back 2 = 사람 화면 · 유령 시트 뒤 Back 1번에 사람 화면(v3.89 FAIL → PASS) · 빠른 연속 Back · v3.84 뒤로가기(기대값: drill 복귀로 갱신)·v3.83/87 차수·v3.86 결제자·v3.89 요약·v3.72/73 오버레이 회귀 PASS, overflow 0·콘솔 에러 0. ✅ 쉼표 모양은 사용자 실기기에서 Dutch Pay와 동일 확인(2026-09-26). 변경 파일 `index.html`·`CLAUDE.md`.
