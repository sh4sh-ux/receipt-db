...
### Changelog
- `v2.69` — **스캔 파일 이름의 '문서'가 매장명으로 등록되던 것 수정.** iOS 메모가 짓는
  `스캔 문서 3.pdf`에서 `_SCAN_NOISE`가 `스캔`만 걸러내고 **`문서`를 남겨 매장명이 '문서'가 됐다.**
  필터에 `문서`·`문서스캔`·`스캔본`·`파일`·`document(s)`·`doc(s)`·`file(s)`·`scanned` 추가.
  ⚠️ 필터는 **낱말 전체 일치**(`^…$`)라 `문서리아 강남점` 같은 진짜 상호는 그대로 남는다 —
  부분일치로 바꾸면 멀쩡한 매장명이 잘려 나가므로 바꾸지 말 것.
  남은 조각이 한 글자면 `store.length>=2` 검사에서 이미 걸러지므로 `새` 같은 건 넣지 않았다.
- `v2.68` — **가게명 브랜드 자동 분류 + '바' 오분류 수정.** `CAT_RULES`에 일반명사(`카페`·`커피`)만
  있어서 실제 영수증 대다수인 **브랜드 상호가 하나도 안 잡혔다** — '스타벅스 강남구청정문점'이
  대표 사례(미결 항목이었다). 원인은 `#r=` 링크 경로가 아니다: 링크도 `updatePreview()`를 거쳐
  `autocategorize()`를 정상 호출하고 `loadStoreCatMap()`도 먼저 await된다. **붙여넣기로 직접
  입력해도 똑같이 실패**했다. 카테고리별 브랜드·일반명사를 보강했다.
  같이 고친 것: 술집의 한 글자 키워드 **`'바'`가 파리바게뜨·폴바셋·바르다김선생을 술집으로
  오분류**하던 버그(`호프`·`포차`·`주점`·`와인` 등으로 대체). 그리고 영문·숫자 키워드는
  **낱말 경계**로 매칭한다(`_kwHit`) — `CU`를 부분일치로 두면 `Circus` 같은 상호에 걸린다.
  ⚠️ 규칙은 **위에서부터 첫 일치**가 채택되므로 순서가 중요하다. `롯데리아`(외식)와
  `롯데시네마`(영화)가 갈리도록 **`롯데` 같은 접두어는 넣지 않는다.** 한 글자 키워드도 금지.
  기본값이 어긋나도 사용자가 한 번 고치면 `storeCatMap`이 학습해 그 가게는 이후 그 값을 쓴다.
...
- **규칙 사전 (`CAT_RULES`)**: `{cat, kws}` 배열. 가게명에 키워드 포함 시 매칭.
  일반명사 + **브랜드 상호**를 함께 둔다(v2.68) — 실제 영수증은 대부분 상호라서 일반명사만으론 안 잡힌다.
  ⚠️ **위에서부터 첫 일치**가 채택되니 순서가 중요하다. `롯데` 같은 접두어는 넣지 말 것
  (`롯데리아`=외식 / `롯데시네마`=영화가 갈려야 한다). **한 글자 키워드도 금지** —
  `'바'`가 파리바게뜨·폴바셋을 술집으로 오분류했다.
  매칭은 `_kwHit()`: 한글은 부분일치, **영문·숫자는 낱말 경계**(`CU` ≠ `Circus`).
...
## 현재 상태 (2026-08-31 기준)
- **버전 `v2.69`** — 브랜치 `claude/claude-md-review-hewk71`에 push 완료. **main 병합 대기 중.**
...
- `v2.68`·`v2.69` 가게명 브랜드 자동 분류 보강 + `'바'` 오분류 제거,
  스캔 파일 이름의 '문서'가 매장명이 되던 것 수정.
...
- **영수증 폴더 *바로 아래*에 넣은 파일은 앱이 보지 않는다.** `_dbxProcessScanbox()`는
  `_dbxScanDir()`(=`…/스캔함`) 한 폴더만 `recursive:false`로 읽는다. v2.46의 '낱개 파일
  스캔함으로 옮기기'는 **옛 폴더(`_LEGACY_WORK_DIR`=`/01_Personal/영수증`) 전용**이고
  설정 탭 '치우기' 버튼을 눌러야만 도는 **수동** 기능이라 새 폴더에는 적용되지 않는다.
- **PDF는 맥(`receipt_pdf_to_jpg.py`)이 켜져 있어야 제대로 등록된다.** 앱은 PDF를 열지 못해
  `총액 0`·품목 없음·사진 없음인 껍데기 레코드만 만든다(메모: '스캔함 자동등록 (PDF — 사진 없음)').
  폰만으로 끝내려면 PDF 대신 **사진(JPG/PNG)** 으로 `스캔함`에 넣어야 한다.
- ⚠️ **사진에서 매장명·금액을 읽는 곳은 어디에도 없다.** 앱에 OCR이 없고,
  맥 스크립트도 마찬가지다(`receipt_png_to_receipt_db.py` 독스트링: "매장명·품목·총액은 비워두므로").
  스캔함이 채워 주는 유일한 단서는 **파일 이름**이다 — 앱 규칙
  (`260825_영수증(2,000)_다이소 강남구청역점.jpg`)에 맞으면 `_parseOwnScanName`이
  날짜·금액·매장명을 통째로 되읽는다. 기기 기본 이름(`사진 2026. 8. 31. 오후 2 26 15.jpg`)이면
  **날짜만, 그것도 사진 찍은 날**로 들어간다(영수증 날짜가 아니다).
...
### 미결
- **상세 화면에 품목 붙여넣기·추가 수단이 없다.** `parseReceiptText()`를 부르는 곳은
  추가 탭 `updatePreview()` 한 군데뿐이다(v2.67 확인). 그런데 CLAUDE.md의 v1.60 항목과
  `receipt_png_to_receipt_db.py` 독스트링은 **아직 '상세 화면 붙여넣기(v1.60)로 채우면 된다'고
  안내한다** — 그 버튼은 지금 없다. 스캔함이 만드는 껍데기 레코드(매장명·총액·품목 빈 값)를
  나중에 채울 방법이 없다는 뜻이라, 설계된 흐름('사진 먼저 → 나중에 채우기')이 끊겨 있다.
  복원 여부 결정 필요.
- ~~`#r=` 링크로 열면 카테고리가 자동 분류되지 않는다~~ → **v2.68에서 해결.**
  링크 경로가 아니라 `autocategorize()`의 브랜드 상호 미등록이 원인이었다.
