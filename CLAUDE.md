...
### Changelog
- `v3.80` — **추가 화면 compact — '항상 펼쳐놓기' → '필요할 때 선택'(모바일 세로 공간 축소, 기능 삭제 없음). ⚠️ `parseReceiptText`·`validateParsed`·저장 경로(`saveBtn`)·`_setAddSaveState`(v3.79 fix)·Receipt schema·계산(`_participantSplit`/`_receiptShare`/splitExclude/treat/treatBy)·Person·Meeting·Relation Group·Review Inbox·Dropbox·migration 전부 불변.**
  ① **함께한 사람**: 기본 화면엔 **선택된 사람만 chip** + `+ 조상현(나)`(나를 빠르게 넣는 버튼 — **자동 선택 아님**, 이미 넣었으면 숨김) + `+ 추가`. chip은 **Receipt Detail과 같은 `_makeSplitUI`** 그대로라 탭=분담↔제외(깍두기)·태그(분담/제외/전액/0원)·🎉 한턱 bar·한턱 주체(관계 그룹 후보 있을 때만 개인/공동, 기본 개인)까지 상세 화면과 동일 interaction. `_makeSplitUI`에 **선택 옵션 2개만 추가**(`meName`=`(나)` 표기·`onRemove`=× 빼기) — 상세 화면은 옵션을 넘기지 않아 동작 불변. 한턱 bar·분담 안내는 참석자 2명↑일 때만 노출(이미 켜졌으면 유지). v3.79의 '최근 6명 추천 chip'·'분담 방식 접힘 row + [일반|한턱|제외] 세그먼트'는 제거.
  ② **사람 picker**(`_qcOpenPicker`, 기존 `_mtgSheetOpen` 재사용 → 데스크탑 중앙·모바일 하단 시트·overlay history/swipe-back 계약 그대로): 검색·초성, 후보 = 나 → 사람 화면 context → 입력된 사람 → 최근 함께한 사람(runtime·캐시 없음), 목록에 없는 이름은 **'직접 추가'**(기존 직접 입력 기능을 picker 안으로 이동). people=여러 명 토글+완료, payer=한 명 선택 즉시 닫힘.
  ③ **결제자** = 1 row(`결제자 조상현(나) ›`, 나=Signature Blue·상대=Green) → picker. 칸이 비면 저장 규칙(`getMyName()`)대로 '나' 표시. `participantsInp`/`payerInp`는 숨김 input으로 남아 저장 경로가 그대로 읽는다(picker는 값을 바꾸고 input 이벤트만 보냄).
  ④ **추가 정보**(접힘 1 row, 요약 `카드 · 카페 · 메모 · 태그 N`) → 결제수단·카테고리(같은 row grammar, 기존 select·카테고리 팝오버 그대로)·경조사 필드·메모·태그. 결제수단·카테고리는 필수 입력이 아니고 붙여넣기·자동 분류로 채워지며 요약에 값이 보여 접음.
  **효과**: 붙여넣기 후 입력 영역 높이 390px 1317→726px · 1280px 1338→836px(옵션 영역 795→204px). Desktop은 레일·왼쪽 영수증 목록·오른쪽 추가 화면 3단 그대로(같은 정보 구조). **검증(실데이터 151 + 관계 그룹 fixture, 영향 범위만)** 390·430·1280 각 17항목 PASS: 기본 화면(선택 0·추천 chip 0·결제자 Blue·추가 정보 접힘·저장 활성), 사람 추가(나 버튼+picker)·직접 추가·× 삭제, 결제자 변경(Green·즉시 닫힘), 일반 분담, 깍두기(chip 탭→splitExclude), 저장→상세 재오픈 chip 동일, 개인 한턱(주체 후보 없음→숨김), 공동 한턱(기본 개인·공동 선택→treatBy 저장·`_receiptShare` 김영석 208,000/조상현 0), 추가 정보 저장(현금·카페·메모·태그), overflow 0·저장 버튼 화면 안·콘솔 에러 0. iPhone Safari 실기기 미검증. 변경 파일 `index.html`·`CLAUDE.md`.
